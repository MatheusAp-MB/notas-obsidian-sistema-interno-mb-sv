...
- Frete CIF/FOB — continua vindo da planilha. Diferente dos outros itens desta lista, aqui falta **investigação**, não só espera de API: ainda não sabemos de onde vem esse dado na prática nem qual a lógica de negócio por trás de CIF vs. FOB — precisa entender isso antes de cogitar tirar da planilha.
...
| ICMS de saída | Planilha — em paralelo, planilha "Busca Legal" (com superior/financeiro) enquanto API de saída não existe |
| PIS/COFINS de saída | Planilha — mesma limitação, mesmo caminho "Busca Legal" |
| Frete CIF/FOB | Planilha — falta investigação (de onde vem o dado, qual a lógica CIF vs. FOB), não só espera de API |
