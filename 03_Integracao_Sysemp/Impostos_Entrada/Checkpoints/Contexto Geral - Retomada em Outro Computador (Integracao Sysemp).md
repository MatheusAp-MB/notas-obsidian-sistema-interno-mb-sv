...
## Status real agora (17/08/2026, 23:58) — momento da pausa

> [!success] A decisão maior de arquitetura (multi-empresa) foi tomada e implementada nesta mesma noite
> Tudo que a seção "Status real agora (17/08/2026, 01:20)" logo abaixo descreve como bloqueado/pendente **foi resolvido depois dela ter sido escrita**. Se você está retomando em outro computador agora, comece por [[Checkpoint - Implementacao de Suporte Permanente a 2 Empresas (Roteamento por Sessao)]] (o que mudou, os bugs corrigidos, a validação real) e por [[Guia de Setup - Do Zero ao Primeiro Preco Calculado]] (passo a passo atualizado) — não pela seção de 01:20 abaixo, que descreve o estado ANTES dessa resolução.

**O que mudou desde a pausa de 01:20, em ordem:**

1. **Arquitetura permanente de 2 empresas implementada e validada** — 2 bancos MySQL completamente separados (`sistema_interno_magazine`/`sistema_interno_samvale`), escolhidos por sessão web (tela "Escolher empresa" + selo fixo) ou por `--empresa=MAGAZINE`/`--empresa=SAMVALE` em qualquer comando de terminal. Substitui de vez o banco temporário (`sistema_interno_sv_temp`) e a variável de ambiente sobrescrita na hora que a seção de 01:20 abaixo descreve. Detalhe completo, incluindo 5 bugs reais corrigidos no caminho (sessão roteada por engano fazendo o badge voltar pra "Magazine" no F5, login não compartilhado entre empresas, migração antiga sem `.using()`, arquivo/token do ERP e Sysemp sem resolução por empresa, jsons de retorno da API sem isolamento por empresa): [[Checkpoint - Implementacao de Suporte Permanente a 2 Empresas (Roteamento por Sessao)]].
2. **Sincronização fiscal da Samvale RODOU DE VERDADE, com token real** — `sincronizar_impostos_entrada --empresa=SAMVALE`: 3586 selecionados, 518 sincronizados, 3068 sem produto correspondente (esperado — só o relatório de Produtos Ativos é importado hoje, ver [[Guia de Setup - Do Zero ao Primeiro Preco Calculado]]), 0 erro. O bloqueio "sem token" registrado na seção de 01:20 e em [[Primeira Importacao Real de Dados da Samvale (SV) — Pipeline Generaliza Sem Mudanca de Logica]] **não existe mais**.
3. **Log do terminal de `sincronizar_impostos_entrada` reescrito** com `rich` — mostra empresa ativa, cobertura de datas já sincronizada, janela sendo buscada agora, tabela de CFOPs mantidos no filtro, tabela de tempo por fase, e painel de resumo final. Validado nas 2 empresas.
4. **Novo comando `calcular_todas_as_grades_precificacao --empresa=X`** — roda os 6 comandos de grade (`calcular_grade_precificacao_ml/tiktok/raia/amazon/magalu/shopee`) em sequência, sem substituir os individuais. Validado nas 2 empresas (1280 produtos Magazine, 592 Samvale).
5. **Tutorial temporário aposentado** — [[Tutorial - Gerar Relatorio de Impostos de Entrada da Samvale (SV) em Banco Temporario]] movido pra `Tutoriais/Obsoletos/`, substituído por [[Guia de Setup - Do Zero ao Primeiro Preco Calculado]] (que já usa `--empresa=`/`--database=`).
6. **Commit real, já em `origin/dev`**: `1a216d6` — `feat: suporte permanente a 2 empresas (MB/SV) via roteamento por sessao e --empresa`. Sincronizado e conferido arquivo por arquivo nesta sessão, bate com o que foi validado.

**Item real ainda não confirmado, verificar amanhã**: os dados fiscais da Samvale já estão no banco de verdade (item 2 acima) — mas não há confirmação registrada nesta conversa de que o relatório `.xlsx` final (tela "Resumo de Impostos de Entrada", `/impostos/`) chegou a ser exportado e entregue ao superior, que era o pedido original de urgência que deu origem a todo este trabalho. Confirmar isso antes de considerar essa tarefa do superior 100% encerrada.
