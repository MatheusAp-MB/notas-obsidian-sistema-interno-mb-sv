...
### Estado real do commit — IMPORTANTE pra troca de computador

Confirmado por `git fetch origin dev` nesta sessão (26/08/2026, 16:35): o repositório remoto segue parado no commit `6707cbf` (ponto 04). **O diff deste ponto 05 (função nova em `classificacao_catalogo.py`, os 2 arquivos novos de `buscar_dados_sku_completo.py`, e a religação de `QUALIDADE`/`COMPETICAO` em `popular_banco.py` + `importar_qualidade_anuncio.py`) foi aplicado e testado de verdade pelo usuário, mas ainda não foi commitado nem enviado pro GitHub.** Ver seção "Próximo passo", mais abaixo — prioridade #1 ao retomar.
...
> [!warning] Atualização de 26/08/2026, 16:35 — plano de 5 pontos 100% migrado e testado. Só faltam 2 ações finais, nenhuma delas é mais migração
> Os 5 pontos (01 tokens, 02 `buscar_mlbs.py`, 03 `buscar_detalhes.py`, 04 `classificar_por_sku.py`/unificação, 05 `buscar_dados_sku_completo.py`) estão **todos migrados, testados e validados** — inclusive na tela real do site (ver seção "Ponto 05 concluído..." acima). O que falta não é mais trabalho de migração, são só 2 ações:
>
> **1. Commitar o diff do ponto 05 — AINDA NÃO FOI FEITO.** Confirmado por `git fetch origin dev` nesta sessão (26/08/2026, 16:35): o repositório remoto (GitHub, branch `dev`) segue parado no commit `6707cbf` (ponto 04). As mudanças do ponto 05 — `encontrar_fecho_transitivo()` migrada pra `mercado_livre/funcoes_auxiliares/classificacao_catalogo.py`, os 2 arquivos novos (`integracao_mercado_livre/servicos/buscar_dados_sku_completo.py` + `management/commands/buscar_dados_sku_completo.py`), e a religação de `QUALIDADE`/`COMPETICAO` em `popular_banco.py` (com `caminho_dados_completos_por_sku()` nova em `importar_qualidade_anuncio.py`) — foram **aplicadas e testadas de verdade pelo usuário** (Magazine e Samvale, 1 SKU cada, confirmado na tela do site), mas **só existem no computador local, não commitadas nem enviadas pro GitHub**. Isso importa MUITO pra troca de computador: se o diretório de trabalho local não for levado junto (ou copiado) pro novo computador, esse trabalho já testado se perde e precisa ser regenerado a partir do diff registrado na seção "Ponto 05 concluído..." acima (o código completo dos 2 arquivos novos não está reproduzido nesta nota — só o resumo técnico; se o diff local se perder mesmo, a conversa que gerou ele tem o código completo). **Prioridade #1 ao retomar: commitar e dar push nisso antes de qualquer outra coisa**, mesmo que a base inteira ainda não tenha rodado.
>
> **2. Rodar a base inteira, nas 2 empresas — não foi feito ainda.** Só foi testado com 1 SKU por empresa. Pra fechar o pipeline de dado completo: `python manage.py buscar_dados_sku_completo --empresa magazine` (sem `--skus`, roda todos os SKUs distintos da base — usuário avisou que isso sozinho pode levar mais de 1h30) e o mesmo pra `--empresa samvale`, e depois `python manage.py popular_banco --empresa MAGAZINE`/`SAMVALE` de novo, pra `QUALIDADE`/`COMPETICAO` gravarem no banco pra base inteira, não só pro SKU de teste.
>
> Depois desses 2 passos, o plano de 5 pontos desta nota está oficialmente fechado — não sobra nenhum arquivo do projeto antigo pra migrar. Único assunto relacionado que segue fora deste plano, pausado à parte: `promocoes_completo.json`/`PROMOÇÕES ML` (nunca fez parte dos 5 pontos).
...
3. ~~Escrever o diff de migração de cada um dos 5 arquivos...~~ — **concluído em 26/08/2026, 16:35**: ver ação 1 do quadro de aviso acima — aplicado e testado, só falta commitar.
4. ~~Só depois de tudo isso, descomentar as 5 linhas dentro da lista `etapas`...~~ — **completado em 26/08/2026, 16:35** (parcialmente antecipado em 15:42): 4 das 5 linhas (`ANUNCIOS ML`, `DIMENSÕES DECLARADAS ML`, `QUALIDADE`, `COMPETICAO`) estão religadas — só `PROMOÇÕES ML` segue comentada (assunto pausado à parte). Falta só rodar a base inteira do ponto 05 e `popular_banco` de novo (ver ação 2 do quadro de aviso acima) pro dado valer pra todos os SKUs, não só o de teste.
