...
## Status real agora (28/08/2026, 17:06)

> [!info] Frente nova, em paralelo ao caso HIDROLIGHT (que segue como estava, ver "Status anterior 20/08" logo abaixo — não foi resolvido, só não é mais o item mais recente)
> A API do Sysemp trouxe o campo **CEST** (só no JSON, nunca persistido). Um terceiro passou pro usuário uma planilha pré-existente da **Samvale** que precisa da Coluna K (CEST) preenchida, cruzando NCM (Coluna A) e EAN/GTIN (Coluna C) já existentes na própria planilha contra o JSON. Escopo explicitamente restrito pelo usuário: **só Samvale por enquanto**. Detalhe completo (chaves reais do JSON, os 2 bugs achados e corrigidos, os 2 mockups aprovados, a regra final das 3 cores) em [[Preenchimento de CEST na Planilha Samvale via Referencia EAN-NCM-CEST do JSON]].

Resumo do que já foi feito, em 3 etapas:

1. **Aba plana de referência** (`EAN_NCM_CEST`) — 1 linha por registro do JSON. Achado real na 1ª rodada: chaves `snake_case` chutadas (`codigo_barras`, `ncm_cadastro`) estavam erradas — as chaves reais são rótulos em português com espaço/acento (`'Código Barras'`, `'NCM Cadastro'`/`'NCM XML'`, `'CEST'`, `'NR NF'`).
2. **Árvore hierárquica** (`Arvore_NCM_CEST_EAN`, NCM → CEST → EAN) — 1ª versão (indentação/outline) foi **rejeitada** pelo usuário como confusa em escala real; substituída por células **realmente mescladas** (`openpyxl.merge_cells()`), com a coluna "NF(s)" trazendo os números reais de `NR NF` (não uma contagem). Bug real achado e corrigido: CEST como string vazia `""` não é `None` pro Python, criava um 2º grupo "sem CEST" espúrio — corrigido normalizando todo valor bruto do JSON na extração.
3. **Cruzamento e preenchimento da Coluna K** — regra de 3 cores: EAN+NCM batendo → verde, EAN batendo com NCM diferente → amarelo (preenche mesmo assim), EAN não encontrado → cinza (não preenche). Resolvida sem ambiguidade porque o JSON confirmadamente nunca repete EAN.

**Status real**: Etapas 1 e 2 validadas com dado real (rodadas pelo usuário). Etapa 3 (`preencher_cest_planilha_samvale.py`) entregue como texto, testada só com dado sintético no sandbox de Claude — **ainda não rodada pelo usuário contra a planilha real**. Toda a frente seguiu a regra já registrada deste domínio: Claude nunca lê/executa contra o dado real do usuário, só entrega código como texto.

## Status anterior (20/08/2026, 19:05) — histórico, ver "Status real agora" acima pro estado atual

> [!success] HIDROLIGHT fechada na prática — status atual: AGUARDANDO RETORNO DO ESCRITÓRIO DE CONTABILIDADE
> O usuário deu entrada nas notas que faltavam (ver "Status anterior" abaixo) e confirmou: **depois disso, o problema da HIDROLIGHT não precisa mais de nenhuma mescla/cross-reference de dado** — a entrada manual das notas que faltavam já resolveu o sintoma. **Importante para não assumir errado**: nenhuma das 2 hipóteses de causa raiz levantadas em 19/08 (filtro de CFOP descartando silenciosamente vs. `selecionar_nota_mais_recente_por_produto()` mantendo só 1 nota por produto) foi formalmente confirmada nesta conversa — o usuário só relatou que o sintoma prático desapareceu depois da entrada manual, não qual das 2 era a causa. Ficam registradas como hipóteses não resolvidas, não como causa raiz decidida, caso o mesmo padrão apareça de novo em outra marca.

### Achado novo — outras marcas com CST PIS/COFINS errado, corrigido por NCM

Depois da HIDROLIGHT, apareceram **outras marcas** com CST PIS/COFINS incorreto no relatório de impostos de entrada. O time fez as consultas necessárias e obteve, junto à contabilidade, uma **tabela de referência validada** com 3 colunas: `NCM (Cadastro)`, `CST PIS`, `CST COFINS`. A correção foi aplicada com um script Python de uso único (não faz parte do pipeline oficial `integracao_sysemp`/`impostos` — é um utilitário pontual, entregue como texto na conversa, nunca como arquivo criado por Claude, seguindo a regra de colaboração deste domínio):

- **Leitura da referência**: `Tabela_NCM_CORRETO.xlsx` (`Arquivos_RELATORIO_IMPOSTOS/`) via `pandas`, construindo um dicionário `{ncm_normalizado: (cst_pis, cst_cofins)}`.
- **Edição em-lugar (não geração de arquivo novo)**: a primeira versão do script usava `pandas.to_excel()` e escrevia um arquivo novo — **rejeitada pelo usuário** porque `to_excel()` reconstrói o arquivo do zero e destrói toda a formatação existente (cores, largura de coluna, formato numérico). Reescrito com `openpyxl.load_workbook()`, alterando só o `.value` de cada célula que precisava mudar e salvando de volta no MESMO arquivo (`Baixado_Relatorio_Impostos_Entrada.xlsx`) — formatação 100% preservada.
- **Normalização**: NCM normalizado pra string numérica de 8 dígitos (remove tudo que não é número + `zfill(8)`); CST normalizado preservando zero à esquerda — 2 dígitos pra CST normal, 3 dígitos pra CSOSN (Simples Nacional) — reaproveitando diretamente a lição já registrada em [[CST Perdia o Zero a Esquerda e Nao Suportava CSOSN]].
- **Não-destrutivo por padrão**: linha cujo NCM não tem correspondência na tabela de referência fica **intocada** (nunca zerada/limpa) — só contabilizada e reportada no console ao final, nunca escrita na planilha.

**Resultado real da execução** (rodado pelo usuário, saída colada na conversa): 47 NCMs únicos carregados como referência; aba usada na planilha a corrigir: `'Impostos de Entrada'`; **76 linhas corrigidas**, **527 linhas já estavam corretas**, **78 NCMs sem correspondência** na tabela de referência (lista completa dos 78 foi passada pelo usuário e inclui um valor anômalo, `'00000000'`, que sugere um NCM ausente/em branco em algum produto — não uma classificação fiscal real).

> [!question] Em aberto — o que fazer com os 78 NCMs sem correspondência
> Perguntado ao usuário se vale compilar essa lista pra enviar de volta ao escritório de contabilidade (pra saber se são NCMs válidos que só não estavam na tabela de referência recebida, ou se é esperado que fiquem de fora). **Não respondido diretamente** — o usuário seguiu direto pro pedido de atualização do vault, com o status geral "aguardando retorno da contabilidade". Não assumir que os 78 estão cobertos por esse status até confirmação explícita.

### Status atual (verbatim do usuário)

> [!success] Confirmação do usuário (20/08/2026, 19:05)
> *"Sobre os impostos eu consegui corrigir o que precisava, e gerei todas as planilhas que precisava. o status atual é: AGUARDANDO RETORNO DO ESCRITÓRIO DE CONTABILIDADE."*

Mesma categoria de bloqueio da pausa de 18/08 (ver [[Pausa do Trabalho de Impostos de Entrada e Multi-Empresa - Foco Exclusivo em Agenda de Videos]]) — dependência de terceiro externo, fora do controle técnico do projeto. Detalhe completo da metodologia e do resultado em [[Correcao de CST PIS e COFINS por NCM Fecha a Frente da Hidrolight e Outras Marcas]].

### Próximo passo real, pra quem retomar (neste ou noutro computador)

1. Aguardar retorno do escritório de contabilidade — sem ação técnica pendente enquanto isso não chega.
2. Quando o retorno chegar, decidir com o usuário o que fazer com os 78 NCMs sem correspondência (pergunta em aberto acima) — inclusive investigar a origem do NCM anômalo `'00000000'`.
3. Confirmar se o script de correção (ou algo equivalente) precisa ser oficializado dentro do pipeline `integracao_sysemp`/`impostos`, ou se continua sendo um utilitário pontual de uso único — não decidido ainda, não assumir nenhum dos dois lados.
4. Os itens 1-6 da seção "Status anterior (19/08/2026, 18:27)" abaixo sobre a investigação HIDROLIGHT (achados 1 e 2, ação de reset de banco) seguem como registro histórico da investigação — não precisam mais de ação, já que o usuário confirmou o sintoma resolvido.

## Status anterior (19/08/2026, 18:27) — momento da pausa, ver "Status real agora" acima pro estado atual
...
- **Rodar `preencher_cest_planilha_samvale.py` contra a planilha real da Samvale e confirmar o resultado** (Etapa 3 do CEST — script entregue 28/08, só testado com dado sintético até aqui) — ver [[Preenchimento de CEST na Planilha Samvale via Referencia EAN-NCM-CEST do JSON]].
