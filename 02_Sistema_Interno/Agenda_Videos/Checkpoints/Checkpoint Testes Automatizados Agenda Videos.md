...
5. Validação física com o ML real (postar/replicar sem clicar) — esclarecido que **não é pytest** (sucesso é visual: mouse parado sobre o botão, sem clique — não dá pra `assert` isso). Script de Postagem entregue, salvo pelo usuário e **commitado**: `scripts_dev/testar_fluxo_real_ml_sem_clicar.py` — baixa vídeo real do Drive (só leitura), abre o navegador de verdade, sobe o arquivo, para o mouse no botão final sem clicar (a função `postar_video_no_ml()` já faz isso por decisão anterior, 30/07). **Ainda não executado.** Script de Replicação (gêmeo) + flag `confirmar_de_verdade=False` em `replicar_video_no_ml()` (necessária porque essa função clicava de verdade, decisão de 30/07) foram desenhados e entregues em texto (06/08, 16:10) — **ainda NÃO aplicados nem criados de fato**, ver [[Flag Temporaria de Confirmacao em Replicar Video no ML]] (guarda o código completo pra não depender da memória da conversa). Plano: usuário aplica os 4 blocos, roda o script de Postagem, depois o de Replicação, e só então volta pro item 6 com as telas reais na cabeça de novo. Corre em paralelo ao pytest, não bloqueia o resto do mapa.
...
12/08/2026 (Sexta rodada — reestruturação das 6 telas de nível igual + 3 arquivos de teste fechados, 23:51) — Navegação da Agenda de Vídeos reestruturada por completo: as telas antigas, baseadas em Fase (Todos/Não Agendado/Simples/Mensal/Trimestral/A Fazer Hoje), foram substituídas por um modelo novo baseado em Período × Etapa — 6 telas de nível igual (Geral/A Fazer Hoje/Aguardando Postar-Replicar/Aguardando Aprovação/Prontos pra Agendar Mensal/Pausados na Agenda). Detalhe completo, incluindo os 2 fixes de design confirmados nesta sessão (chip "concluído" que faltava em Geral; A Fazer Hoje Simples só entra com Base já feita) e o redesenho dos botões de automação de Postar/Replicar, em [[Reestruturacao da Navegacao da Agenda de Videos em 6 Telas de Nivel Igual]] (nota nova — substitui [[Estrutura de Telas da Agenda de Videos]], agora `descartada`). Suíte de teste fechada pro modelo novo: `test_nivel_3__listar_produtos_agenda_filtrados.py` reescrito por completo (100% cover, 0 Miss, 0 BrPart em `filtros_agenda_videos.py` — achou e corrigiu 1 bug real de teste + fechou 3 lacunas de branch coverage); 4 testes corrigidos em `test_nivel_4__view_agenda_videos.py` (Tela.TODOS/Tela.SIMPLES → Tela.GERAL/Periodo.SIMPLES); arquivo novo `test_nivel_4__contexto_tela_agenda_videos.py` fecha `contexto_tela_agenda_videos.py` de 83% pra 100% cover, introduzindo `RequestFactory` no projeto pela 1ª vez. **Confirmado: 520 passed, 0 failed, 12 xfailed** (suíte inteira). Commit (título+descrição) já gerado pro usuário aplicar — ver seção "Sexta rodada" abaixo pro detalhe completo.
...
## Sexta rodada — Reestruturação das 6 Telas de Nível Igual + Fechamento de Testes (a partir de 12/08/2026)

Motivada por 2 ajustes de design confirmados com o usuário: faltava o chip "Prontos pra agendar mensal" na tela Geral, e A Fazer Hoje perdia sentido mostrando qualquer Simples ativo, sem noção real de urgência. Virou reestruturação completa da navegação — modelo Fase-based (Todos/Não Agendado/Simples/Mensal/Trimestral/A Fazer Hoje) substituído por modelo Período×Etapa (Geral/A Fazer Hoje/Aguardando Postar-Replicar/Aguardando Aprovação/Prontos pra Agendar Mensal/Pausados). Detalhe completo, incluindo a ordem corrigida de `OPCOES_ETAPA`, a regra nova de escopo de A Fazer Hoje (Mensal/Trimestral em qualquer etapa; Simples só com Base feita) e o redesenho dos botões de automação, em [[Reestruturacao da Navegacao da Agenda de Videos em 6 Telas de Nivel Igual]].

### Testes fechados nesta rodada

- **`test_nivel_3__listar_produtos_agenda_filtrados.py` — reescrito por completo** (9 blocos: mecânico, escopo das 6 telas, `condicao_etapa()`, A Fazer Hoje, Aguardando Postar/Replicar, Aguardando Aprovação, Prontos pra Agendar, Pausados, Geral/Período/Etapa, filtros compartilhados). **100% cover, 0 Miss, 0 BrPart em `filtros_agenda_videos.py`** (144 stmts, 72 branches). 1 bug real de teste (não de produção) corrigido: o cenário de Aguardando Postar/Replicar chamava `listar_produtos_agenda_filtrados()` (sempre estreita pra 1 aba) esperando ver a união bruta de postar+replicar — corrigido pra chamar `construir_queryset_tela()` direto. 3 testes novos fecharam lacunas de branch coverage nos filtros "sim"+"não" marcados ao mesmo tempo (atrasado/risco/sincronizado_drive — nenhum dos dois filtra, comportamento intencional sem teste dedicado até então).
- **`test_nivel_4__view_agenda_videos.py` — 4 testes corrigidos** (referenciavam `Tela.TODOS`/`Tela.SIMPLES`, enum antigo). Achado semântico: `Tela.GERAL` **calcula** contadores de chip (dict de 8 chaves zeradas pra produto sem cache) — diferente da antiga `Tela.TODOS`, que não tinha chip nenhum (`== {}`). 1 bug de setup no meio do caminho: SKU de teste longo demais gerava `ean` (`f'EAN-{sku}'`) acima do `max_length=20` — corrigido encurtando o SKU.
- **`test_nivel_4__contexto_tela_agenda_videos.py` — novo.** Fecha `contexto_tela_agenda_videos.py` de 83% pra **100% cover**. Introduz `RequestFactory` no projeto pela 1ª vez — testa `ContextoTelaAgendaVideos` isolado, sem HTTP nem template (confirmado: nenhum cenário precisa de `ConfiguracaoFase` semeada, já que `calcular_diagnostico_preparo_drive` é stub atualmente). Cobre os 5 métodos privados de querystring de link, os 4 fallbacks de valor inválido na querystring, o bloco `simular_data` (só com `DEBUG=True`, incluindo o ramo "ausente" que exigiu 1 teste extra depois de 1ª rodada com 99% cover), os 3 rótulos do chip de faixa, e os 3 ramos de `_montar_contadores_chips()` + o `{}` das 3 telas sem chip.

**Confirmado: 520 passed, 0 failed, 12 xfailed** (suíte inteira) — único item nessa contagem que não é desta rodada é a ausência total de falha na `api_sysemp`, que tinha 1 falha conhecida pré-existente (variável de ambiente do teste com nome errado, corrigida nesta mesma sessão — ver [[Teste de ApiSysemp Monkeypatchava Variavel de Ambiente com Nome Errado]], mundo `03_Integracao_Sysemp`, sem relação de domínio com a Agenda de Vídeos).

### Commit gerado

Título e descrição completos gerados nesta sessão pro usuário aplicar — cobre os 4 arquivos de teste acima (rewrite + fixes + arquivo novo) e a correção do nome de variável no teste do Sysemp.
...
**Pausa (06/08/2026, 16:10) — projeto novo e paralelo, sem relação com Agenda de Vídeos, vai começar.** Este é o ponto exato de retomada: código real em `2388b1f` (branch `dev`, confirmado no GitHub — inclui os itens 1-4 da Rodada 6, 357 passed). Os 4 blocos de diff da flag temporária de Replicação (item 5) e o script novo foram entregues em texto mas **ainda não aplicados** — o código completo está guardado em [[Flag Temporaria de Confirmacao em Replicar Video no ML]], não só nesta conversa. Vault comitado localmente neste mesmo bloco — falta o usuário dar `git push`. Pra retomar: (1) aplicar os 4 blocos, (2) rodar os 2 scripts manuais de dry-run (Postagem já existe, Replicação é novo), (3) só então decidir onde exibir o percentual de replicação (item 6, [[Percentual de Replicacao por Produto e Geral]]).
