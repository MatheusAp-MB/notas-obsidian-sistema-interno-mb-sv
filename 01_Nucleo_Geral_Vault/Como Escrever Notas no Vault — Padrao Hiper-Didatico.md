...
relacionado: [Guia de Setup - Do Zero ao Primeiro Preco Calculado, Aviso Proativo Para Notas no Obsidian, Princípios Fundamentais do Vault]
resumo: Toda nota deve ser escrita em Modo Professor — explicando pra quem nunca viu o assunto, nunca como colega experiente resumindo pra outro colega experiente — seguindo 8 regras práticas (nunca abreviar, sempre com exemplo concreto, comando sempre em bloco de código próprio) e um checklist de verificação antes de considerar a nota pronta.
---
...
Decisão explícita do usuário (16/08/2026, 22h25): a exigência de escrita didática já registrada em "Estrutura e Convenções do Vault" (seção "Escrita didática", decidida originalmente em 15/08/2026 às 01:39 — quando o vault deixou de ser só o "HD" de memória do Claude, e o time, com Cauã confirmado lendo notas no mesmo dia, passou a buscar informação direto aqui também) não é mais suficiente — precisa ser reforçada e detalhada ao ponto de garantir, **com 100% de certeza**, que qualquer pessoa que leia uma nota entenda plenamente o que foi escrito, sem margem de dúvida nenhuma.
...
Esta nota substitui, na prática, a antiga seção "Escrita didática" que existia em "Estrutura e Convenções do Vault" — essa nota original foi depois extraída em 9 notas de responsabilidade única e apagada (30/08/2026, ver [[Registro dos Mundos Ativos]] e as demais notas de `01_Nucleo_Geral_Vault/`); a convenção de escrita didática vive inteiramente aqui, sem precisar apontar pra mais nenhum lugar.
...
### As 8 regras práticas

Toda nota precisa seguir estas 8 regras (cada uma demonstrada com um par ANTES × DEPOIS na seção "Exemplo", logo abaixo):
...
A alternativa descartada foi escrever "como um colega experiente resumindo pra outro colega experiente" — o jeito mais natural e mais rápido de escrever, mas que só funciona pra quem já sabe do que se trata. Foi rejeitada porque falha justamente no momento em que mais importa: quando o time lê o vault sem contexto prévio, ou quando Claude relê a nota depois de uma conversa compactada, sem lembrar de nada além do que está escrito ali. Regra sem exemplo é regra abstrata — por isso cada uma das 8 regras práticas vem com um par ANTES/DEPOIS: mostrar o jeito proibido lado a lado com o jeito exigido deixa a diferença impossível de interpretar errado, em vez de descrever a regra só em teoria.

## Exemplo

Cada uma das 8 regras práticas, com o "ANTES" (jeito vago, proibido a partir de agora) e o "DEPOIS" (jeito exigido):
...
| **Citação de fonte** | Toda afirmação sobre o código real | "arquivo `X.py`, função `Y()`, linha Z" — nunca "o sistema faz X" solto, sem dizer onde |
